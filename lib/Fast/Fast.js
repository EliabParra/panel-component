export const Fast = class extends HTMLElement{
    ...
    async getCssFile(fileName) {
        if (!this.cssFiles.has(fileName)) {
            let css = await fetch(`${fast.routes.css}/${fileName}/${fileName}.css`).then((response) => response.text());
    ...
    async getSVG(fileName) {
        if(!this.svgFiles.has(fileName)){
            let svg = await fetch(`${fast.routes.icons}/${fileName}/${fileName}.svg`).then((response) => response.text());
